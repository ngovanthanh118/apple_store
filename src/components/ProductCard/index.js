    ...
    return (
        <div className="flex flex-col justify-center items-center bg-[#323232] rounded-2xl cursor-pointer overflow-hidden box-product">
            <div className="relative ">
                <img className="img-product cursor-pointer" src={"https://apple-store-server.vercel.app/api/v1/images/" + product.image[0]} alt='' onClick={() => navigate(`/product/${product.url}/${product._id}`)} />
            ...
            <div className="mobile-add-cart flex justify-between items-center px-3 pb-3">
                {product?.discount > 0 ?
                    <div className="flex gap-2 items-center">
                        <h1 className="font-bold text-xl text-white">${product.discount}</h1>
                        <p className="font-normal text-base text-white line-through">${product.price}</p>
